...
public class Memory extends Observable {
    ...
    /**
     * Get a Term for a given name of a Concept or Operator, called in StringParser
     * and the make methods of compound terms.
     * 
    ...
    private void derivedTask(Task task) {
        if (task.aboveThreshold()) {
            ...
            if (budget > minSilent)
                report(task.getSentence(), ReportType.OUT); // report significient derived Tasks
            newTasks.add(task);
        }
    }
    ...
    public void executedTask(Task task) { // called by the inference rules
        ...
        float minSilent = parameters.SILENT_LEVEL / 100.0f;
        if (budget > minSilent)
            report(task.getSentence(), ReportType.EXE);
    }
    ...
    /**
     * Shared final operations by all double-premise rules, called from the rules
     * except StructuralRules
     * 
     * @param budget  The budget value of the new task
    ...
    private void processTask() {
        ...
        while (counter-- > 0) { // process the newTasks of the previous cycle
            ...
            if (task.getSentence().isInput() || (termToConcept(task.getContent()) != null)) // new input or existing
                                                                                            // concept
    ...
    private void continuedProcess(Task task, Term content) {
        ...
        if (content instanceof CompoundTerm) {
            ...
            cLinks = (c1 != null) ? c1.getTermLinks() : ((CompoundTerm) content).prepareComponentLinks(this); // use
                                                                                                              // saved
            short[] indices;
